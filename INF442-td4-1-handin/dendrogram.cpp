...
void dendrogram::merge(edge *e) {
    ...
    // 4. Update ranks
    // 5. Update heights
    int parent1 = find(e->get_p1());
    int parent2 = find(e->get_p2());
    if (parent1 != parent2){
        // make parent1 be the one with highest rank (or highest index)
        ...
        left[parent2] = down[parent1];
        down[parent1] = parent2;

        if (rank[parent1] == rank[parent2]){
            rank[parent1]++;
        }
        height[parent2] = e->get_length()/2;
    }
}
...
int dendrogram::_count_ns_clusters() {
    int count = 0;
    int nb_nodes = g->get_num_nodes();
    int count_arr[nb_nodes];

    for(int i = 0; i < nb_nodes; i++){
        count_arr[i] = 0;
    }
    for(int i = 0; i < nb_nodes; i++){
        if(count_arr[clusters[i]] == 0){
            count_arr[clusters[i]]++;
        }
        else if(count_arr[clusters[i]] == 1){
            count_arr[clusters[i]]++;
            count++;
        }
    }
